...
set_attribute(model, "limits/gap", 0.03)
...
XLSX.openxlsx("upper_level_decisions.xlsx", mode = "w") do xf
    sheet = xf[1]
    XLSX.rename!(sheet, "UpperLevelVars")

    # Time index
    T = length(JuMP.value.(λ_VPPp))

    # Header
    sheet["A1"] = "Time"
    sheet["B1"] = "λ_VPPp (Buy Price)"
    sheet["C1"] = "λ_VPPs (Sell Price)"
    sheet["D1"] = "P_DSO_p"
    sheet["E1"] = "P_DSO_s"
    sheet["F1"] = "z1 (Buy Flag)"
    sheet["G1"] = "z2 (Sell Flag)"

    for t in 1:T
        sheet[t+1, 1] = t
        sheet[t+1, 2] = JuMP.value(λ_VPPp[t])
        sheet[t+1, 3] = JuMP.value(λ_VPPs[t])
        sheet[t+1, 4] = JuMP.value(P_DSO_p[t])
        sheet[t+1, 5] = JuMP.value(P_DSO_s[t])
        sheet[t+1, 6] = JuMP.value(N_1[t])  # z1 in paper notation
        sheet[t+1, 7] = JuMP.value(N_2[t])  # z2 in paper notation
    end
end